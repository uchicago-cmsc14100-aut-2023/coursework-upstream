...
def load_json_safe_test(filename):
    '''
    Loads a ngram count or language model test file from JSON. These tests 
    require special handling of tuple dict keys, as JSON does not support 
    tuples as dict keys, they were serialized to str when tests were created.

    Paramters:
        - filename: Path to JSON file to be loaded
    
    Returns:
        - Deserialied contents of JSON file. Any dictionary keys that were 
          encoded as string will be converted to tuples.
    '''
    data = None
    ...
    for test_case in data:
        json_expected_dict = test_case[-1]
        expected_dict = {}
        for key, value in json_expected_dict.items():
            if key.startswith('('):
        ...
        new_test_case = test_case[:-1] + [expected_dict]
...
def check_dict_values(actual, expected, unordered_values=True, recreate_msg=None):
    ...
    Parameters
        ...
        - expected: the expected dictionary
        - recreate_msg: a message that can be used to recreate the test
        - unordered_values: if True, the values in the actual and expected are 
                            not considered to be ordered and will be compared
    ...
    for key in expected:
        ...
        elif unordered_values and isinstance(expected[key], list):
            msg = check_unordered_1d_iterable(actual[key], expected[key], recreate_msg)
        else:
...
def check_unordered_1d_iterable(actual, expected, recreate_msg):
    '''
    This function is used to check tuples and lists. Generates an error if the 
    actual iterable is not the same as the expected (ignoring order). 
    Return None if no problem is found. Elements of the iterable must be
    hashable for this function to work.
    ...
    # Check if there's a mismatch between the actual and expected lists, 
    # order ignored

    if set(actual) != set(expected):
        msg = (f"The actual list ({actual}) does not match the expected list\n")
        msg += (f"Expected List: {expected}\nActual List: {actual}")
        msg += (f"Difference between the two: {set(actual) ^ set(expected)}")
        if recreate_msg is not None:
            msg +=  '\n' + recreate_msg
        return msg
